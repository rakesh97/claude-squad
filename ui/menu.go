...
var defaultMenuOptions = []keys.KeyName{keys.KeyNew, keys.KeyPrompt, keys.KeyImport, keys.KeyHelp, keys.KeyQuit}
...
func (m *Menu) addInstanceOptions() {
	...
	if m.instance != nil && m.instance.Status == session.Loading {
		m.options = []keys.KeyName{keys.KeyNew, keys.KeyImport, keys.KeyHelp, keys.KeyQuit}
		return
	}

	// Instance management group
	options := []keys.KeyName{keys.KeyNew, keys.KeyImport, keys.KeyKill, keys.KeyRename}
